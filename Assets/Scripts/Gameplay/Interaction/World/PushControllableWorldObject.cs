...
namespace Gameplay.Interaction.World
{
    public class PushControllableWorldObject : MonoBehaviour, IControlable, IPressable
    {
        ...
        public bool CanPress(GameObject interactor, in PointerTargetContext pointerContext)
        {
            ...
            return ResolvePushDirection(interactor).sqrMagnitude > Epsilon && _actionStepDistance > Epsilon;
        }
        ...
        public void Press(GameObject interactor, in PointerTargetContext pointerContext)
        {
            ...
            var pushDirection = ResolvePushDirection(interactor);
            ...
            {
                InteractionDebugLog.Log(this, $"Push press blocked on '{name}' by '{blockerName}'.");
                return;
            }
            ...
            InteractionDebugLog.Log(this, $"Push press executed on '{name}'. stepDistance={_actionStepDistance:0.###}, targetPosition={nextPosition}, duration={_actionMoveDuration:0.###}.");
        }
        ...
        public bool CanControl(ControlMode mode, GameObject interactor, in PointerTargetContext pointerContext)
        {
            ...
            return ResolvePushDirection(interactor).sqrMagnitude > Epsilon;
        }
        ...
        public void BeginControl(ControlMode mode, GameObject interactor, in PointerTargetContext pointerContext)
        {
            ...
            _isControlled = true;
            _activeMode = mode;
            InteractionDebugLog.Log(this, $"Push control began on '{name}' by '{interactor.name}'. origin={_originPosition}.");
            _onControlStarted?.Invoke();
        ...
        public void UpdateControl(ControlMode mode, GameObject interactor, in PointerTargetContext pointerContext)
        {
            ...
            if (moveDelta.sqrMagnitude > Epsilon && IsMovementBlocked(moveDelta, out var blockerName))
            {
                InteractionDebugLog.LogVerbose(this, $"Push control blocked on '{name}' by '{blockerName}'.");
                return;
            }
        ...
        private Vector3 ResolvePushDirection(GameObject interactor)
        {
            if (interactor == null)
            {
                return Vector3.zero;
            ...
            if (fromInteractor.sqrMagnitude <= Epsilon)
            {
                return Vector3.zero;
            }

            return _ignoredMovementAxes.Apply(fromInteractor, Epsilon);
        }
